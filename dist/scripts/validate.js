...
function formatTime(date, timeZone) {
    if (!date)
        return 'N/A';
    return luxon_1.DateTime.fromJSDate(date).setZone(timeZone).toFormat('yyyy-MM-dd HH:mm');
}
function validateTithiTimes() {
    ...
    testDates.forEach(date => {
        const panchangam = (0, panchangam_1.getPanchangam)(date, observer);
        const dateStr = luxon_1.DateTime.fromJSDate(date).toFormat('yyyy-MM-dd (EEEE)');
