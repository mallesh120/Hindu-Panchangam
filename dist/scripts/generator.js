...
exports.generateHtmlCalendar = generateHtmlCalendar;
const astronomy_engine_1 = require("astronomy-engine");
const panchangam_1 = require("../panchangam");
...
function formatTime(date, timeZone) {
    if (!date)
        return '';
    return luxon_1.DateTime.fromJSDate(date).setZone(timeZone).toFormat('HH:mm');
}
function generateHtmlCalendar(year, month, observer, timeZone) {
    const monthDate = luxon_1.DateTime.fromObject({ year, month });
    ...
    const firstDayOfMonth = luxon_1.DateTime.fromObject({ year, month, day: 1 }).weekday; // 1 for Monday, 7 for Sunday
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const sunriseToday = (0, astronomy_engine_1.SearchRiseSet)(astronomy_engine_1.Body.Sun, observer, 1, date, 300);
        ...
        const sunriseTomorrow = (0, astronomy_engine_1.SearchRiseSet)(astronomy_engine_1.Body.Sun, observer, 1, nextDate, 300);
        ...
        const p1 = (0, panchangam_1.getPanchangam)(dayWindow.start.date, observer);
        ...
        let yogaHtml = `${panchangam_1.yogaNames[p1.yoga]} (until ${formatTime(p1.yogaEndTime, timeZone)})`;
        if (p1.yogaEndTime && p1.yogaEndTime < dayWindow.end.date) {
            const nextYogaIndex = (p1.yoga + 1) % 27;
            yogaHtml += `<br>${panchangam_1.yogaNames[nextYogaIndex]} (starts ${formatTime(p1.yogaEndTime, timeZone)})`;
        }
        calendarHtml += `
            <div class="day">
